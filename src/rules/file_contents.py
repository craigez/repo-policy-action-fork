...
def run(
    ...
    if not matched_files:
        if fail_on_missing:
            return reporter.rule_failed(
                ...
                or f"No files matched patterns {globs}",
            )
        return reporter.rule_passed(
    ...
    for file_path in matched_files:
        ...
        if skip_binary and mime_detector and _is_binary(
            file_path, mime_detector
        ):
            logger.debug(
        ...
        if not _file_contains(file_path, compiled, line_count):
            return reporter.rule_failed(
                ...
                message=fail_message
                or (
                    f"Pattern {content_pattern!r} not found in {rel}"
                ),
                file_path=rel,
            )
...
def _should_skip_path(
    rel_path: str, skip_patterns: list[re.Pattern]
) -> bool:
